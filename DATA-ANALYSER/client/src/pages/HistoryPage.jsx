    ...
    <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
      <button onClick={() => setOpen(!open)}
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-stone-400 text-xs">{new Date(createdAt).toLocaleString()}</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex gap-4 text-xs">
            <span className="text-stone-500">Leads: <span className="text-stone-800 font-bold">{summary.totalLeads}</span></span>
            <span className="text-teal-600">Qualified: <span className="font-bold">{summary.qualified}</span></span>
            <span className="text-blue-600">Connect: <span className="font-bold">{summary.connectPct}%</span></span>
          </div>
          <div className="flex gap-1" onClick={e => e.stopPropagation()}>
            <button onClick={() => exportToExcel({ ...result, detectedColumns }, baseName)}
            ...
            <button onClick={() => exportToPDF({ ...result, detectedColumns }, baseName)}
      ...
      {open && (
        <div className="border-t border-stone-100 p-5 space-y-4 bg-stone-50">
          ...
          <div>
            <p className="text-stone-500 text-xs font-semibold uppercase tracking-wider mb-2">Day-wise</p>
            <div className="flex flex-wrap gap-2">
              {result.dayAnalysis?.map(r => (
                <span key={r.day} className="bg-teal-50 border border-teal-200 rounded-lg px-3 py-1 text-xs text-teal-800">
                  {r.day}: <span className="font-bold">{r.connect}</span> connects
                </span>
              ))}
            </div>
          </div>
          ...
          <div>
            <p className="text-stone-500 text-xs font-semibold uppercase tracking-wider mb-2">Column Mapping Used</p>
            <div className="flex flex-wrap gap-2">
              {Object.entries(detectedColumns || {}).map(([field, col]) => (
                <span key={field} className="bg-white border border-stone-200 rounded-lg px-3 py-1 text-xs text-stone-600">
                  {field} → <span className="text-stone-800 font-semibold">{col}</span>
                </span>
              ))}
            </div>
          </div>
    ...
    <div className="p-6 space-y-5 max-w-screen-xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-0.5">{history.length} records found</p>
        </div>
        <div className="flex bg-white border border-stone-200 rounded-xl p-1 gap-1 shadow-sm">
          {FILTERS.map(({ label, value }) => (
            <button key={value} onClick={() => setFilter(value)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${
                filter === value ? "bg-teal-600 text-white shadow-sm" : "text-stone-500 hover:text-stone-800"
              }`}>
              {label}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white border border-stone-200 rounded-2xl">
          <p className="text-stone-400 text-sm">No uploads found for this period</p>
        </div>
      ) : (
